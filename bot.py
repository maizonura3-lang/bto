"""
Bot Scalping v24.0 — BACK TO NORMAL LOGIC
====================================================
- Logika berlawanan dihapus (kembali Normal: Indikator LONG = Bot LONG)
- SL = 0.4% dan TP = 0.3% (Sesuai instruksi)
- Server-Side Bracket Orders tetap aktif
"""
...
def live_open(orig_direction, score, sigs, price, atr, regime, bias, sym):
    ...
    print(f"\n  {d} [v24.0 NORMAL] {sym} {actual_side} @{price:.6g} | Strict SL:{SL_PCT*100:.1f}% TP:{TP_PCT*100:.1f}%")
    _stats["trades"] += 1
...
def process_closed_position(sym, pos_data, reason, current_price):
    ...
    print(f"  {e} [v24.0 NORMAL] {sym} {side} CLOSED by {reason}")
...
def print_inline():
    ...
    print(f"       ┌ [v24.0 NORMAL] {n}T WR:{wr:.0f}% W:{_stats['wins']} L:{_stats['losses']} {e}PnL:{pnl:+.4f}U")
...
def print_full():
    ...
    e = "💚" if pnl >= 0 else "🔴"
    print(f"\n  {'─'*70}")
    print(f"    ✅ SERVER-SIDE v24.0 — NORMAL LOGIC (0.4% SL / 0.3% TP)")
...
def run_bot():
    print("╔════════════════════════════════════════════════════════════════════╗")
    print("║  ✅ NORMAL DIRECTION v24.0 — BACK TO ORIGINAL LOGIC                ║")
    print("║  ✅ Searah dengan indikator | Strict TP 0.3% | SL 0.4%             ║")
    print("╚════════════════════════════════════════════════════════════════════╝")
    try:
