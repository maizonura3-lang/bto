"""
Bot Scalping v21.0 — REFINED & NATIVE EXCHANGE ORDERS
====================================================
- SL/TP langsung di-set di server Binance (No Slippage/Jebol)
- Strict Risk: SL 0.4% dan TP 0.3% (Netto 0.5% / 0.2% w/ fees)
- Real-time Position Synchronization (Anti bocor max_positions)
"""
...
# Strict Risk Management (Sesuai perhitungan + Fee)
SL_PCT = 0.004  # 0.4% Max SL (Total 0.5% dengan round-trip fee)
TP_PCT = 0.003  # 0.3% TP (Total 0.2% profit netto dengan round-trip fee)
...
# ═══════════════════════════════════════════════════════════════════════════
#  MARKET REGIME & SIGNALS (UNMODIFIED FROM YOUR LOGIC)
# ═══════════════════════════════════════════════════════════════════════════
...
_stats = {
    ...
    "extreme_tp": 0, "hard_sl": 0, "active_binance_positions": 0,
...
def live_open(orig_direction, score, sigs, price, atr, regime, bias, sym):
    ...
    # Logic REVERSED: If orig is LONG, actual is SHORT
    actual_side = "SHORT" if orig_direction == "LONG" else "LONG"
    
    # Calculate STRICT Percentage SL & TP
    ...
    # Apply proper precision for Binance API
    ...
    try:
        ...
        # 1. OPEN MARKET ORDER
        order = client.futures_create_order(symbol=sym, side=api_side, type="MARKET", quantity=q_val)
        
        # 2. SET STOP LOSS DI SERVER
        client.futures_create_order(
            symbol=sym, side=close_side, type="STOP_MARKET", 
            stopPrice=sl_price, closePosition="true"
        )
        
        # 3. SET TAKE PROFIT DI SERVER
        client.futures_create_order(
        ...
        try: client.futures_cancel_all_open_orders(symbol=sym) # Cleanup jikalau gagal di tengah
        except: pass
        with _lock: live_positions.pop(sym, None)
        return
...
def process_closed_position(sym, pos_data, reason, current_price):
    ...
    fee_rate = 0.0005 # fee standar Binance futures
    total_fee = (entry * q_val + current_price * q_val) * fee_rate
...
def monitor_positions():
    """ Menggunakan API Binance langsung untuk memonitor apakah Order TP/SL kesentuh """
    try:
        # Ambil posisi real-time dari Binance
        positions = client.futures_position_information()
        active_binance_syms = []
        
        for p in positions:
            if float(p["positionAmt"]) != 0:
                active_binance_syms.append(p["symbol"])
                
        # Update tracker global agar bot ngga open melebihi batas
        _stats["active_binance_positions"] = len(active_binance_syms)

        # Cek apakah posisi lokal kita sudah ditutup oleh Binance (Kena SL/TP bracket)
        with _lock:
            local_syms = list(live_positions.keys())
            
        for sym in local_syms:
            pos_data = live_positions[sym]
            if pos_data.get("_r"): continue # Masih proses open
            
            if sym not in active_binance_syms:
                # Posisi sudah tidak ada di Binance = Kena TP atau SL!
                with _lock: live_positions.pop(sym, None)
                
                # Coba bersihkan sisa order TP / SL di server Binance
                try: client.futures_cancel_all_open_orders(symbol=sym)
                except: pass
                
                # Analisa kasar berdasarkan current price
                px = price_live(sym)
                if pos_data["side"] == "LONG":
                    reason = "TP Hit" if px > pos_data["entry"] else "SL Hit"
                else:
                    reason = "TP Hit" if px < pos_data["entry"] else "SL Hit"
...
def print_inline():
    ...
    print(f"       ┌ [v21.0 SERVER SL/TP] {n}T WR:{wr:.0f}% W:{_stats['wins']} L:{_stats['losses']} {e}PnL:{pnl:+.4f}U")
...
def print_full():
    ...
    e = "💚" if pnl >= 0 else "🔴"
    print(f"\n  {'─'*70}")
    print(f"    ✅ SERVER-SIDE SL/TP v21.0 — FIXED RISK (0.4% SL / 0.3% TP)")
...
def run_bot():
    print("╔════════════════════════════════════════════════════════════════════╗")
    print("║  ✅ REVERSED LOGIC v21.0 — SERVER-SIDE BRACKET ORDERS              ║")
    print("║  ✅ Strict 0.4% SL | 0.3% TP | No Slippage | Auto-Sync Positions   ║")
    print("╚════════════════════════════════════════════════════════════════════╝")
    try:
    ...
    # Init posisi pertama kali (sinkronisasi awal kalau habis restart)
    try:
        positions = client.futures_position_information()
        act = len([p for p in positions if float(p["positionAmt"]) != 0])
        _stats["active_binance_positions"] = act
    except: pass
    ...
    while True:
        ...
        if (k := ks_check())[0]:
            ...
            print(f"  ✅ Slots full (Synched with Binance)")
        else:
